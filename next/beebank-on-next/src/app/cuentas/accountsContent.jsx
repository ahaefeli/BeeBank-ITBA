      ...
      {Object.keys(data).map((user) => (
        <div className={styleAccounts.account}>
          ...
          <div className={styleAccounts.accountTransferInfo}>
            ...
            <label className={`${styleAccounts.accountCbu} number_format`}><i className="bi bi-clipboard cbu-copy" />{data[user].alias}<label className={styleAccounts.beebankTag}>{(user.bank=="Beebank")?'$bee':null}</label></label>
          </div>
